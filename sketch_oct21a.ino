...
int kar=0;int kar1=0;int kar2 = 0;int x=0;int final;int count=0;int present_count=0; int current_row; int current_column; int current_dir;int min_dir;        
int **arr;  
int link[50]; 
float fdist;    float rdist;    float ldist;  float bdist;      // front, right, and left distances.
float fdur[11];  float rdur[11];  float ldur[11];  float bdur[11];    // Arrays which store the values of five durations... We will take only the median value(afer sorting) with error bearing capacity of 40%.                      
int fblocks;int rblocks;int lblocks;int bblocks;

 
float middleval(float mid[])
{
   for(kar2=0;kar2<10;kar2++)
    {
        for(kar1=0;kar1<10;kar1++)
        {
...
void dijkstras(){
  ...
  while(count!=0){
    count--;
    kar = (link[count]/100)%100; 
    kar1 = (link[count]%100); 
    if(arr[kar][kar1]%10>0){   
      if(kar1==0)
        break;
      if(((arr[kar][kar1]/10000)<(arr[kar][kar1-1]/10000)) || (arr[kar][kar1-1]/10000==0 && !(kar==6 && kar1-1==8))){
        arr[kar][kar1-1] += ((arr[kar][kar1]/10000)+1)*10000;
        link[count] = kar*100 + (kar1-1)*1;
        count++;
      }
               
    }
    if((arr[kar][kar1]/10)%10>0){     
      if(kar==14)
        break;
      if(((arr[kar][kar1]/10000)<(arr[kar+1][kar1]/10000)) || (arr[kar+1][kar1]/10000==0 && !(kar+1==6 && kar1==8))){
        arr[kar+1][kar1] += ((arr[kar][kar1]/10000)+1)*10000;
        link[count] = (kar+1)*100 + kar1*1;
        count++;
      }
       
    }
    if((arr[kar][kar1]/100)%10>0){    
      if(kar1==14)
        break;
      if(((arr[kar][kar1]/10000)<(arr[kar][kar1+1]/10000)) || (arr[kar][kar1+1]/10000==0 && !(kar==6 && kar1+1==8))){
        arr[kar][kar1+1] += ((arr[kar][kar1]/10000)+1)*10000;
...
void loop(){
  if(count>=1){
    ...
    else if(rblocks>0 && rblocks<8){
      current_dir = 1;
    }
    else if(bblocks>0 && bblocks<8){
      current_dir = 2;
    }
    else if(lblocks>0 && lblocks<8){
      current_dir = 3;
    }
    //led(current_dir);
    go(current_dir);
    ...
    current_dir = (link[0]%10)%4;
    current_column = (link[0]/10)%100;
    current_row  = (link[0]/1000)%100;

    count = 0;
    dijkstras();    
    present_count = 0;
    while( !(current_row==6 && current_column==8)    ){
      if(current_row!=0){
        kar2 = arr[current_row-1][current_column]/10000 ;   
        kar = current_row-1;
        kar1 = current_column;
        min_dir = current_dir;
      }
      if(current_column!=14 && arr[current_row][current_column+1]/10000<kar2){
        kar2 = arr[current_row][current_column+1]/10000;
        kar = current_row;
        kar1 = current_column+1;
        min_dir =  (current_dir + 1)%4 ;  
      }
      if(current_row!=14 && arr[current_row+1][current_column]/10000<kar2){
        kar2 = arr[current_row+1][current_column]/10000;
        kar = current_row-1;
        kar1 = current_column;
        min_dir =  (current_dir + 2)%4 ;  
      }
      if(current_column!=0  && arr[current_row][current_column-1]/10000<kar2){
        kar2 = arr[current_row][current_column-1]/10000;  
        kar = current_row;
        kar1 = current_column-1;
        min_dir =  (current_dir + 3)%4 ;  
      }
      link[present_count] = min_dir;
      current_row = kar;
      current_column = kar1;
      current_dir = min_dir;
      present_count++; 
    ...
    while(present_count!=0 ){
      //led(link[present_count]); 
      go(link[present_count]);   
      present_count--;
    }
