...
int kar=0;int kar1=0;int kar2 = 0;int x=0;int final;int count=0;int present_count=0; int current_row; int current_column; int current_dir;int step_count = 0;       
int **arr;  
int link[50]; 
...
void loop(){
  if(count!=1){
    ...
    arr[current_row][current_column] += 100000;
    int i = abs(8-current_row)+abs(6-current_column);
    int min = 100;    
    int min_dir;
    int k = arr[current_row][current_column] ;
    int d = k%10;    
    int c = (k/10)%10;
    int b = (k/100)%10;
    int a = (k/1000)%10;    
    while(i!=0){
      if(a>0 && a<8){
        if(arr[current_row][current_column-1]/100000==0 && min>abs(8-current_row)+abs(7-current_column)){
          min = abs(8-current_row)+abs(7-current_column);min_dir = 0;
        }
      }
      if(b>0 && b<8){
        if(arr[current_row+1][current_column]/100000==0 && min>abs(7-current_row)+abs(6-current_column)){
          min = abs(7-current_row)+abs(6-current_column);min_dir = 1;
        }
      }
      if(c>0 && c<8){
        if(arr[current_row][current_column+1]/100000==0 && min>abs(8-current_row)+abs(5-current_column)){
          min = abs(8-current_row)+abs(5-current_column);min_dir = 2;
        }
      }
      if(d>0 && d<8){
        if(arr[current_row-1][current_column]/100000==0 && min>abs(9-current_row)+abs(6-current_column)){
          min = abs(9-current_row)+abs(6-current_column);min_dir = 3;
        }
      }
      current_row = current_row - (min_dir%2)*(min_dir-2);     
      current_column = current_column - (min_dir%2-1)*(min_dir-1);     
      i = min;
      min = 100;    
      arr[current_row][current_column] += 100000;
      k = arr[current_row][current_column] ;
      d = k%10;
      c = (k/10)%10;    
      b = (k/100)%10;
      a = (k/1000)%10;
      link[step_count] = min_dir;
      step_count++;
    }
    Serial.print("Step_Count is ");
    Serial.println(step_count);    
    int y = 0;
    while(y!=step_count){
      current_dir = (link[y] - current_dir +4)%4;                 
      Serial.println("current_dir is ");
